...
async def init_bot(client: Client = client):
    ...
    if app_config.agent and app_config.agent_code_awareness:
        try:
            from waku.plugins.agent.tools import init_code_repository
    ...
    group_admin_commands = [
        ...
        BotCommand("clean", "Xoá tin nhắn đang reply"),
        BotCommand("cleanall", "Xoá tin nhắn bot sau mốc reply"),
    ]
    private_commands = [
    ...
    owner_commands = [
        ...
        BotCommand("info", i18n.t("bot.cmd.info", locale=app_config.lang)),
        BotCommand("config", i18n.t("bot.cmd.config", locale=app_config.lang)),
    ]
...
async def stop_bot(client: Client = client):
    ...
    if app_config.agent and app_config.agent_code_awareness:
        try:
            from waku.plugins.agent.tools import close_code_repository
