...
_FALLBACK_TEXT = {
    "bot.button.chat_panel": "⚙️ Mở bảng cấu hình Waku",
    "bot.msg.panel_unavailable": "Mini App chưa được cấu hình hoặc chưa sẵn sàng.",
    "bot.msg.chat_panel": "Mở bảng cấu hình Waku cho nhóm này:",
}


def _t(key: str, lang: str) -> str:
    text = i18n.t(key, locale=lang)
    return _FALLBACK_TEXT.get(key, text) if text == key else text
...
async def panel_group_cmd(client: pyrogram.Client, message: pyrogram.types.Message):
    ...
    if button is None:
        await message.reply(text=_t("bot.msg.panel_unavailable", lang))
        return

    await message.reply(
        text=_t("bot.msg.chat_panel", lang),
