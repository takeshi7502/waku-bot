...
def _match_pattern(rel_str: str, pattern: str) -> bool:
    ...
    # For patterns like "waku/plugins/extra", match "waku/plugins/extra/file.py"
...
async def init_code_repository(
    ...
    agent = await AgentFS.open(AgentFSOptions(id="waku-codebase"))
    ...
    # Load Python files into agentfs (only from waku directory, exclude venv)
    loaded_count = 0
    skipped_count = 0

    waku_dir = PROJECT_ROOT / "waku"
    if not waku_dir.exists():
        logger.error("waku directory not found")
        return agent

    for py_file in waku_dir.rglob("*.py"):
