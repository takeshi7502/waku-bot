...
async def list_my_code_files(
    ...
    Use when:
    - User asks what you can do -> list "/waku/plugins"
    - Looking for a specific feature's location

    Args:
        path: Directory path (e.g., "/waku/plugins"). Use "/" for root.
...
async def read_my_code_file(
    ...
    Args:
        path: File path (e.g., "/waku/plugins/help.py").
...
async def get_my_codebase_overview(
    ...
    try:
        ...
        plugins = []
        try:
            plugin_entries = await list_files("/waku/plugins", include_dirs=True)
        ...
        return {
            "project_name": info.get("project_name", "waku-bot"),
